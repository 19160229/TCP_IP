...
//    and ports. This sample is Windows 2000 or later.
//
//
...
int main(int argc, char **argv)
{
    ...
    //
    // Set the SIO_RCVALL xxx ioctl
    //
	printf("WSAIoCtl....\n");
    optval = RCVALL_SOCKETLEVELONLY ;   //只有它有效???　

    //if (WSAIoctl(s, dwIoControlCode, &optval, sizeof(optval),
     //   NULL, 0, &dwBytesRet, NULL, NULL) == SOCKET_ERROR) 
	//或者
	 if (ioctlsocket(s,SIO_RCVALL,&optval)==SOCKET_ERROR) 
	{
        printf("WSAIoctl(%x) failed; %d\n", dwIoControlCode,
            WSAGetLastError());
        return -1;
    }
	printf("will recv IP package...\n");
